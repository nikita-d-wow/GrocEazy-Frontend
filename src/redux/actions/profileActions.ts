...
import type { IAddress, IUser } from '../types/authTypes';
...
export const updateProfile = (data: { name: string; phone: string }) => {
    return async (dispatch: Dispatch) => {
        try {
            const { data: responseData } = await api.put<any>('/api/users/profile', data);
...
export const updateAddress = (addressId: string, addressData: Partial<IAddress>) => {
    return async (dispatch: Dispatch<any>) => {
        try {
            // Guide: PUT /api/users/address/:addressId
            const { data } = await api.put<any>(`/api/users/address/${addressId}`, addressData);
